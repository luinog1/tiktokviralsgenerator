...
| `INSTAGRAM_PROXY` | (vazio) | Proxy (`http://user:pass@host:porta`) usado **só** nas chamadas do Instagram sem token — IPs de datacenter caem no muro de login do site. Vazio → conexão direta (ou `HTTPS_PROXY`, se definido) |
| `INSTAGRAM_PROXY_INSECURE` | `false` | Desliga a validação de TLS **só** nas chamadas do Instagram — portas-proxy de agregadores (ScrapeOps etc.) interceptam o TLS por design e exigem isso |
| `SCRAPEDO_TOKEN` | (vazio) | Token do [Scrape.do](https://scrape.do): as mesmas chamadas do Instagram saem pelo gateway deles (proxies residenciais, `super=true`, 10x créditos). Vence o `INSTAGRAM_PROXY` quando os dois existem |
